...
void SXAudio::setVideo(bool video) {
    isVideo = video;
}

void SXAudio::setClock(int secds) {
    now_time = secds;
    clock = secds;
}

void SXAudio::release() {
    pause();
    if (queue != NULL) {
        queue->noticeThread();
    }
...
void pcmBufferCallBack(SLAndroidSimpleBufferQueueItf bf, void *context) {
    ...
    if (sxAudio != NULL) {
        ...
        sxAudio->buffer = NULL;
        sxAudio->pcmsize = sxAudio->getPcmData(&sxAudio->buffer);
...
int SXAudio::getPcmData(void **pcm) {
    while (!sxPlayStatus->exit) {
        isExit = false;

        if (sxPlayStatus -> pause) //暂停
        {
            av_usleep(1000 * 100);
            continue;
        }
...
//第二步，创建混音器
    ...
    SLDataFormat_PCM pcm = {
            ...
            2,//2个声道（立体声）
            (unsigned int) getSLSampleRate(),//44100hz的频率

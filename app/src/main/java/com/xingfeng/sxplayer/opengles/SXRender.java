...
public class SXRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private RenderRefreshListener mRenderRefreshListener;
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        Log.d(LogTag.TAG, "onSurfaceChanged, width:" + width + ",height :" + height);
        sWidth = width;
        sHeight = height;
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if(cutimg) {
            ...
            if(mSurfaceViewCreateListener != null) {
                mSurfaceViewCreateListener.onCutVideoImg(bitmap);
            }
        }
    }
    ...
    public void setOnRenderRefreshListener(RenderRefreshListener renderRefreshListener) {
        mRenderRefreshListener = renderRefreshListener;
    }

    public void setSurfaceViewCreateListener(SurfaceViewCreateListener surfaceViewCreateListener) {
        mSurfaceViewCreateListener = surfaceViewCreateListener;
    }
    ...
    public Bitmap cutBitmap(int x, int y, int w, int h) {
    ...
    private void renderStop() {
        ...
        GLES20.glVertexAttribPointer(aPositionHandle_stop, 3, GLES20.GL_FLOAT, false, 12, vertexBuffer);
    ...
    private void initMediacodecShader() {
        ...
        if (mSurfaceViewCreateListener != null) {
            mSurfaceViewCreateListener.onGlSurfaceViewOncreate(surface);
        }
    }
}
